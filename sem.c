...
11. программа, которая атомарно подсчитывает и выводит количество запусков (fork -- тоже запуск) в текстовом (fscanf) виде (считая текущий) в файле 11counter.txt
...
Чтобы читать правильное значение из файла, нужно ставить монопольную блокировку разово на чтение и запись.
    ...
    В зависимости, где написать wait, сначала child works, потом parent works, либо наоборот.
Перед каждым вызовом lockf rewind -- двигать offset в начало файла (в этой задаче блокируем весь файл, поэтому lockf(, , 0))
