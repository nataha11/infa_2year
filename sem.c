...
&11. программа, которая атомарно подсчитывает и выводит количество запусков в текстовом (fscanf) виде (считая текущий) в файле 11counter.txt
Корректно обрабатывать параллельные запуски многих экземпляров 
    (в двух терминалах; видим, что вторая программа висит, пока первая не снимет блокировку с файла)
