...
mknod -- создавать block или character device или FIFO
в FIFO содержимого нет, создать FIFO с именем argv[2]
...
Можно использовать fstatat; либо в начале проверка argc = 3 и chdir в argv[1] и 6.1

&6.3 6.1 через open, getdents64, close
...
решать через inotify (fanotify, dnotify, kqueue(freeBSD), ...)
создается очередь событий
переименованный файл IN_MOVED_FROM, IN_MOVED_TO (одни не одновременны, сначала один, через время второй, связываются cookies)
новый файл IN_CREATE

//файловые блокировки
// advisory locks (рекомендательные)
// mandatory (принудительные)
shared(reader lock) -- неисключающая (разделяемая), exclusive (writer lock) -- исключающая (монопольная). 
Читателей м б несколько, писатель -- только один
flock(2) 
...
- если снимает блокировку с файлового дескриптора, то слетают все блокировки с одинаковым fd
lockf(3) 
- можно блокировать области, тогда можно несколько писателей, которые пишут в разные участки файла
//fcntl-lock

&11. программа, которая атомарно подсчитывает и выводит количество запусков в текстовом виде (считая текущий) в файле 11counter.txt
fd, fs -> fscanf()
Корректно обрабатывать параллельные запуски многих экземпляров 
Чтобы читать правильное значение из файла, нужно ставить монопольную блокировку разово на чтение и запись.
    Потом другой процесс будет читать уже увеличенное значение.
Note: перед вызовом lockf каретка должна быть в начале файла (в этой задаче блокируем весь файл, поэтому lockf(, , 0))
Значение неубывающее, поэтому можно не очищать файл, а просто перезаписывать сверху
Данные из буфера записываются на диск при вызове fclose(), но лучше записать сразу при помощи fflush(3)


// процессы
PID -- process identifier (ID)
    находит место в таблице, кусок памяти, куда процесс будет класть свои данные; определяет стандартные потоки ввода и вывода; передает управление на точку входа в программу
...
//мониторить parent_pid, если он изменился, то родитель умер 
busybox
parent pid на 1 меньше текущего, когда родитель жив 
когда parent умирает, pid дочернего меняется
ptrace -- будет долго, но заботаешь больше
...
///пайпы
пайп -- потоковая передача, в каком порядке быди записаны, в таком и прочтены. Хотим отправлять данные сообщениями (чтобы данные не смешивались, если несколько читателей\писателей). Сообщение мы либо прочтем целиком, либо получим ошибку (по кускам нельзя).
Очереди сообщений -- глобальные объекты. Будем рассматривать только позиксовые очереди сообщений (в книге карпова конькова разобраны скорее всего только старые)
